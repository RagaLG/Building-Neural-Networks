...
public class PoolingEdge extends Edge {
    ...
    private void setDelta(double poolMax, int img, int ch, int startCol, int startRow, double[][][][] inputValues){
        for(int poolCol = 0; poolCol < poolSize; poolCol++){
            for(int poolRow = 0; poolRow < poolSize; poolRow++){
                if(inputValues[img][ch][startCol*stride + poolCol][startRow*stride + poolRow] == poolMax){
                    poolDelta[img][ch][startCol*stride + poolCol][startRow*stride + poolRow] += 1;
                }
            }
        }
    }
    ...
    public void propagateForward(double[][][][] inputValues) {
        ...
        int p = outputNode.padding;
        for(int img = 0; img < outputNode.inputValues.length; img++){
            for(int ch = 0; ch < outputNode.inputValues[img].length; ch++){
                for(int col = 0; col < outputNode.inputValues[img][ch].length - 2*p; col++){
                    for(int row = 0; row < outputNode.inputValues[img][ch][col].length - 2*p; row++){
                        double poolMax = Double.NEGATIVE_INFINITY;
                        for(int poolCol = 0; poolCol < poolSize; poolCol++){
                            for(int poolRow = 0; poolRow < poolSize; poolRow++){
                                double value = inputValues[img][ch][col*stride + poolCol][row*stride + poolRow];
                                if(value > poolMax){ 
                                    poolMax = value; 
                                }
                            }
                        }
                        outputNode.inputValues[img][ch][col + p][row + p] = poolMax;
                        setDelta(poolMax, img, ch, col, row, inputValues);
                    }
                }
            }
        }
    ...
    public void propagateBackward(double[][][][] delta) {
        ...
        for(int img = 0; img < delta.length; img++){
            for(int ch = 0; ch < delta[img].length; ch++){
                for(int col = 0; col < delta[img][ch].length; col++){
                    for(int row = 0; row < delta[img][ch][col].length; row++){
                        for(int poolColumn = 0; poolColumn < poolSize; poolColumn++){
                            for(int poolRow = 0; poolRow < poolSize; poolRow++){
                                inputNode.delta[img][ch][col* stride + poolColumn][row * stride + poolRow] = delta[img][ch][col][row]*poolDelta[img][ch][col*stride + poolColumn][row*stride + poolRow];
                            }
                        }
                    }
                }
